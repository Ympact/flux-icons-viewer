...
class PublishFluxIconsVendorFileCommmand extends Command
{
    ...
    public function handle()
    {
        ...
        if ($this->confirm("To use the vendor file, you will need to adjust the namespace in the config file. Do you want to do this now?")) {
            // replace the namespace in the config file
            $configPath = config_path('flux-icons.php');
            $content = file_get_contents($configPath);
            dump(Str::of($oldNamespace)->finish("\\{$className}"),Str::of($newNamespace)->start('\\')->finish('\\'.$className));
            $content = str_replace(
                Str::of($oldNamespace)->finish('\\'.$className)->toString(), 
                Str::of($newNamespace)->start('\\')->finish('\\'.$className)->toString(), 
                $content
            );
